...
evals %>%
  summarise(means = mean(score), medians = median(score), min = min(score), max = max(score), sds = sd(score),
            q1 = quantile(score, probs = 0.25), q3 =quantile(score, probs=0.75))
```
...
evals %>%
  ...
  ylab("Average Prof Evaluation") +
  ggtitle("Scatter plot of average beauty rating to average evaluation rating")
```
...
**The linear model is: score = 3.88 + 0.07 * Beauty Rating.**
...
**For an average beauty increased by 1, the average revaluation rating goes up by 0.07**
...
**For an average beauty rating of 0 the average evaluation rating is 3.88. This doesn't make sense in the data because there aren't zero.**
...
**A male professor scores 0.14 higher on evaluations than a female professor on average. On average, a female professor scores 4.09 on the ratings.**

10. What is the equation of the line corresponding to male professors?
    What is it for female professors?

**For males: score = 4.09 + 0.14 = 4.23**

**For Females: score = 4.09**
...
**The average rating for a teaching track professor is 4.28. The average rating for a tnure track professor is 0.12 points lower than a teaching track prof. The average rating for a tenured prof is 0.14 lower than a teaching prof.**
...
**The model is: score = 4.14 + 0.14 * teaching + 0.02 *Tenure track**

**The average rating for a tenured prof is 4.14. The average rating for a teaching prof is 0.14 higher than a tenured prof. The average rating for a tenure track prof is 0.02 points higher than a tenured prof.**
